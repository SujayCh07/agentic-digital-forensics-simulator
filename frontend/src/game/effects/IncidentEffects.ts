...
/** Spawn a floating text that drifts up and fades. */
...
/** Critical incident marker used by severe events. */
export function spawnCriticalAlert(
  scene: Phaser.Scene,
  worldX: number,
  worldY: number,
) {
  const txt = scene.add
    .text(worldX, worldY - 8, "!! NODE DOWN", {
      fontSize: "11px",
      color: "#ff4444",
      fontFamily: "monospace",
      stroke: "#000000",
...
/** Negative change marker */
export function spawnMoneyLoss(
  scene: Phaser.Scene,
  worldX: number,
  worldY: number,
) {
  floatText(scene, worldX, worldY, "LOAD +", "#ff8800", 14);
}

/** Positive change marker */
export function spawnMoneyGain(
  scene: Phaser.Scene,
  worldX: number,
  worldY: number,
) {
  floatText(scene, worldX, worldY, "PATCH +", "#44ff88", 14);
}

/** Phase change flash with sentiment-driven color */
