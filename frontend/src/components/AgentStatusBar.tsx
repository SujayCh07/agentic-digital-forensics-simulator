...
export function AgentStatusBar({ agents, activeTasks, lockedAgents = [] }: AgentStatusBarProps) {
  return (
    <div className="flex gap-2 h-full items-stretch">
      {agents.map((agent) => {
        ...
        const isActive = !isLocked && agent.status !== "idle" && agent.status !== "standby";

        return (
          <div
            key={agent.id}
            className="rpg-panel flex flex-col justify-between px-2.5 py-1.5 min-w-[130px]"
            style={{
              borderColor: isLocked ? "#1e3d5a" : isActive ? `${agent.color}40` : "#1e3d5a",
              opacity: isLocked ? 0.45 : 1,
              boxShadow: isActive ? `0 0 8px ${agent.color}20, inset 0 1px 0 ${agent.color}10` : undefined,
            }}
          >
            ...
            <div className="flex items-center justify-between gap-1.5 mb-1">
              ...
              {isLocked ? (
                <span className="text-[8px]" style={{ color: "#2a5070" }}>🔒</span>
              ) : (
                <div
