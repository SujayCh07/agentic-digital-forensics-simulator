...
import { u as listDeliverableMessageChannels } from "./message-channel-DKMzUOS9.js";
...
import { t as buildMemoryPromptSection } from "./memory-state-CsGd2Smq.js";
import { n as normalizeStructuredPromptSection, t as normalizePromptCapabilityIds } from "./prompt-cache-stability-BZRcxqbl.js";
import { n as SILENT_REPLY_TOKEN } from "./tokens-fmnHbQeT.js";
import { i as resolveUserTimezone, r as resolveUserTimeFormat, t as formatUserTime } from "./date-time-efBmLE3E.js";
import { t as SYSTEM_PROMPT_CACHE_BOUNDARY } from "./system-prompt-cache-boundary-DweC5y2D.js";
import { t as findGitRoot } from "./git-root-DBjeY33R.js";
...
function buildAgentSystemPrompt(params) {
	...
	const extraSystemPrompt = typeof params.extraSystemPrompt === "string" ? normalizeStructuredPromptSection(params.extraSystemPrompt) : void 0;
	...
	const skillsPrompt = typeof params.skillsPrompt === "string" ? normalizeStructuredPromptSection(params.skillsPrompt) : void 0;
	const heartbeatPrompt = typeof params.heartbeatPrompt === "string" ? normalizeStructuredPromptSection(params.heartbeatPrompt) : void 0;
	...
	const workspaceNotes = (params.workspaceNotes ?? []).map((note) => normalizeStructuredPromptSection(note)).filter(Boolean);
	const modelAliasLines = (params.modelAliasLines ?? []).map((line) => normalizeStructuredPromptSection(line)).filter(Boolean);
	if (promptMode === "none") return "You are a personal assistant operating inside OpenClaw.";
	const lines = [
		...
		] : [`For long waits, avoid rapid poll loops: use ${execToolName} with enough yieldMs or ${processToolName}(action=poll, timeout=<ms>).`, `For long-running work that starts now, start it once and rely on automatic completion wake when it is enabled and the command emits output or fails; otherwise use ${processToolName} to confirm completion, and use it for logs, status, input, or intervention.`],
		...hasUpdatePlanTool ? [
		...
		modelAliasLines.length > 0 && !isMinimal ? "Prefer aliases when specifying model overrides; full provider/model is also accepted." : "",
	...
	if (!isMinimal) lines.push("## Silent Replies", `Use ${SILENT_REPLY_TOKEN} ONLY when no user-visible reply is required.`, "", "⚠️ Rules:", "- Valid cases: silent housekeeping, deliberate no-op ambient wakeups, or after a messaging tool already delivered the user-visible reply.", "- Never use it to avoid doing requested work or to end an actionable turn early.", "- It must be your ENTIRE message - nothing else", `- Never append it to an actual response (never include "${SILENT_REPLY_TOKEN}" in real replies)`, "- Never wrap it in markdown or code blocks", "", `❌ Wrong: "Here's help... ${SILENT_REPLY_TOKEN}"`, `❌ Wrong: "${SILENT_REPLY_TOKEN}"`, `✅ Right: ${SILENT_REPLY_TOKEN}`, "");
...
//#endregion
export { buildSystemPromptParams as i, sanitizeForPromptLiteral as n, wrapUntrustedPromptDataBlock as r, buildAgentSystemPrompt as t };
