import { a as formatUncaughtError } from "./errors-DcAstocv.js";
...
import { A as hasFlag, D as getPositiveIntFlagValue, E as getFlagValue, L as isValueToken, T as getCommandPositionalsWithRootOptions, k as getVerboseFlag } from "./logger-ChPQoo5e.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-C-1dPqbo.js";
import { a as parseCliContainerArgs, i as maybeRunCliInContainer, n as applyCliProfileEnv, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-2aYnnuKC.js";
import { n as defaultRuntime } from "./runtime-BXaxArxm.js";
import { a as enableConsoleCapture } from "./subsystem-DX_oTata.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-WgjbeXMN.js";
import { t as assertSupportedRuntime } from "./runtime-guard-N7-RsVc_.js";
...
import { o as hasMemoryRuntime } from "./memory-state-CsGd2Smq.js";
import "./logging-CAD-eG9-.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-BX9gj_hd.js";
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-DXdb2JCU.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-BF0j104p.js";
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-4A5Wma15.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-Bpnl3L6b.js");
			...
			const { statusCommand } = await import("./status-DrdQt_ir.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-HK46ZSQP.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-DwdJEkMJ.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-D3LCN1Av.js");
	...
	"config-get": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigGet } = await import("./config-cli-xZhmEi3g.js");
	...
	"config-unset": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigUnset } = await import("./config-cli-xZhmEi3g.js");
	...
	"models-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsListCommand } = await import("./models-CcXtLLm0.js");
	...
	"models-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsStatusCommand } = await import("./models-CcXtLLm0.js");
...
async function prepareRoutedCommand(params) {
	...
	const { VERSION } = await import("./version-Bt7AtnlL.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-DUNnZL9N.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-BQxrHycy.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-EzibHaTS.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-ZPMvR1bY.js");
		...
		const [{ buildProgram }, { installUnhandledRejectionHandler }, { restoreTerminalState }] = await Promise.all([
			import("./program-DLymkgNH.js"),
			import("./unhandled-rejections-D_2wr4FE.js"),
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			const { getProgramContext } = await import("./program-context-Cx5YpeZu.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BQaNMfAJ.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-L4dgIFy-.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { registerPluginCliCommandsFromValidatedConfig } = await import("./cli-s5mN7LTG.js");
