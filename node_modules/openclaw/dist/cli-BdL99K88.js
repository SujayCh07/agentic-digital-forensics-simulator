import { i as normalizeLowercaseStringOrEmpty, r as lowercasePreservingWhitespace, s as normalizeOptionalString } from "./string-coerce-CIT1-v_a.js";
import { o as normalizeSingleOrTrimmedStringList } from "./string-normalization-IdjIlZzV.js";
import { h as resolveDefaultAgentId, y as resolveSessionAgentId } from "./agent-scope-BfV3ML7Z.js";
import { s as listActiveMemoryPublicArtifacts } from "./memory-state-CsGd2Smq.js";
import "./text-runtime-dYMrRdFH.js";
import { t as buildPluginConfigSchema } from "./config-schema-C7tGFgOZ.js";
import "./memory-host-core-UK85R6UJ.js";
import { n as withTrailingNewline, t as replaceManagedMarkdownBlock } from "./memory-host-markdown-m06_74qL.js";
import { n as getActiveMemorySearchManager } from "./memory-host-search-LUC8LdVw.js";
import { t as zod_exports } from "./zod-BbINiS6U.js";
import "./api-BAQ_uOMh.js";
...
function normalizeTextKey(text) {
	return normalizeLowercaseStringOrEmpty(text).replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ");
}
...
function slugifyWikiSegment(raw) {
	return normalizeLowercaseStringOrEmpty(raw).replace(/[^a-z0-9]+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "") || "page";
}
...
function scoreDigestClaimMatch(claim, queryLower) {
	let score = 0;
	if (normalizeLowercaseStringOrEmpty(claim.text).includes(queryLower)) score += 25;
	if (normalizeLowercaseStringOrEmpty(claim.id).includes(queryLower)) score += 10;
	if (typeof claim.confidence === "number") score += Math.round(claim.confidence * 10);
...
function buildDigestCandidatePaths(params) {
	...
	return params.digest.pages.map((page) => {
		...
		if (idLower.includes(queryLower)) score += 20;
		if (page.sourceIds.some((sourceId) => normalizeLowercaseStringOrEmpty(sourceId).includes(queryLower))) score += 12;
		...
		}).toSorted((left, right) => scoreDigestClaimMatch(right, queryLower) - scoreDigestClaimMatch(left, queryLower));
...
async function getMemoryWikiPage(params) {
	...
	if (shouldSearchWiki(effectiveConfig)) {
		...
		if (page) {
			const slice = parseWikiMarkdown(page.raw).body.split(/\r?\n/).slice(fromLine - 1, fromLine - 1 + lineCount).join("\n");
			return {
				corpus: "wiki",
				path: page.relativePath,
				title: page.title,
...
function resolveBridgePagePath(params) {
	...
	return {
		...
		pagePath: path.join("sources", `bridge-${workspaceSlug}-${artifactSlug}.md`).replace(/\\/g, "/"),
		workspaceSlug,
		artifactSlug
	};
}
...
//#endregion
export { WIKI_SEARCH_BACKENDS as A, probeObsidianCli as C, runObsidianSearch as D, runObsidianOpen as E, normalizeMemoryWikiMutationInput as F, getMemoryWikiPage as I, searchMemoryWiki as L, memoryWikiConfigSchema as M, resolveMemoryWikiConfig as N, lintMemoryWikiVault as O, applyMemoryWikiMutation as P, compileMemoryWikiVault as R, syncMemoryWikiImportedSources as S, runObsidianDaily as T, runWikiStatus as _, runWikiCompile as a, renderMemoryWikiStatus as b, runWikiIngest as c, runWikiObsidianCommandCli as d, runWikiObsidianDailyCli as f, runWikiSearch as g, runWikiObsidianStatus as h, runWikiBridgeImport as i, WIKI_SEARCH_CORPORA as j, ingestMemoryWikiSource as k, runWikiInit as l, runWikiObsidianSearch as m, runWikiApplyMetadata as n, runWikiDoctor as o, runWikiObsidianOpenCli as p, runWikiApplySynthesis as r, runWikiGet as s, registerWikiCli as t, runWikiLint as u, runWikiUnsafeLocalImport as v, runObsidianCommand as w, resolveMemoryWikiStatus as x, buildMemoryWikiDoctorReport as y, initializeMemoryWikiVault as z };
