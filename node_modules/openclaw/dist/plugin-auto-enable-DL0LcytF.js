...
import { n as loadPluginManifestRegistry, r as resolveManifestContractOwnerPluginId } from "./manifest-registry-_lSkajSy.js";
import { t as isBlockedObjectKey } from "./prototype-keys-BWjW0VW8.js";
import { t as resolvePluginSetupAutoEnableReasons } from "./setup-registry-B4foTAMM.js";
import { a as normalizePluginsConfig, s as resolveEffectivePluginActivationState } from "./config-state-HDtzsRYk.js";
import { r as withBundledPluginVitestCompat } from "./bundled-compat-UbHjJ4bk.js";
import { n as listPotentialConfiguredChannelIds, t as hasPotentialConfiguredChannels } from "./config-presence-DnNFIU-O.js";
import { t as isChannelConfigured } from "./channel-configured-C_h__rLT.js";
import { t as ensurePluginAllowlisted } from "./plugins-allowlist-C8YSv1lS.js";
...
function resolveDiscoveredProviderPluginIds(params) {
	...
	return registry.plugins.filter((plugin) => {
		...
		if (!shouldFilterUntrustedWorkspacePlugins || plugin.origin !== "workspace") return true;
		...
		return normalizedConfig.enabled && !normalizedConfig.deny.includes(plugin.id) && normalizedConfig.allow.includes(plugin.id) && normalizedConfig.entries[plugin.id]?.enabled === false;
...
function shouldSkipPreferredPluginAutoEnable(params) {
	for (const other of params.configured) {
		...
		if (resolvePreferredOverIds(other.pluginId, params.env, params.registry).includes(params.entry.pluginId)) return true;
	}
	return false;
}
//#endregion
...
function resolveProviderPluginsWithOwnedWebSearch(registry) {
	return registry.plugins.filter((plugin) => plugin.providers.length > 0).filter((plugin) => (plugin.contracts?.webSearchProviders?.length ?? 0) > 0);
}
...
//#endregion
export { resolveDiscoveredProviderPluginIds as a, resolveOwningPluginIdsForProvider as c, resolveCatalogHookProviderPluginIds as i, withBundledProviderVitestCompat as l, configMayNeedPluginAutoEnable as n, resolveEnabledProviderPluginIds as o, resolveBundledProviderCompatPluginIds as r, resolveOwningPluginIdsForModelRefs as s, applyPluginAutoEnable as t };
