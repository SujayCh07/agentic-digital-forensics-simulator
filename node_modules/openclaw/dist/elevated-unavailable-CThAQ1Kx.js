...
function extractExplicitGroupId(raw) {
	...
	const channelId = normalizeChannelId(parts[0] ?? "") ?? normalizeOptionalLowercaseString(parts[0]);
	const parsed = (channelId ? getLoadedChannelPlugin(channelId)?.messaging ?? getBundledChannelPlugin(channelId)?.messaging : void 0)?.parseExplicitTarget?.({ raw: trimmed }) ?? null;
