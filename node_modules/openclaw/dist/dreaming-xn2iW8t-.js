...
import { o as parseUsageCountedSessionIdFromFileName } from "./artifacts-DZNRQQib.js";
import "./text-runtime-dYMrRdFH.js";
import { n as appendMemoryHostEvent } from "./events-CxgXFKgA.js";
import { B as resolveMemoryLightDreamingConfig, F as resolveMemoryDeepDreamingConfig, M as formatMemoryDreamingDay, P as resolveMemoryCorePluginConfig, V as resolveMemoryRemDreamingConfig, z as resolveMemoryDreamingWorkspaces } from "./dreaming-Bf_5J43g.js";
import "./memory-core-host-status-DvNy8R93.js";
...
import "./memory-host-events-Bu8pq-E0.js";
import { n as withTrailingNewline, t as replaceManagedMarkdownBlock } from "./memory-host-markdown-m06_74qL.js";
import { a as readShortTermRecallEntries, c as recordShortTermRecalls, i as rankShortTermPromotionCandidates, n as applyShortTermPromotions, o as recordDreamingPhaseSignals, u as repairShortTermPromotionArtifacts } from "./short-term-promotion-7Kjwnqku.js";
import { n as normalizeTrimmedString, t as includesSystemEventToken } from "./dreaming-shared-vsDF3BVq.js";
import { t as generateAndAppendDreamNarrative } from "./dreaming-narrative-ByhshfSa.js";
...
async function collectSessionIngestionBatches(params) {
	...
	for (const file of sortedFiles) {
		...
		const cursorAtEnd = previous !== void 0 && previous.lineCount > 0 && previous.lastContentLine >= previous.lineCount;
...
function buildManagedDreamingPatch(job, desired) {
	...
	if (normalizeLowercaseStringOrEmpty(normalizeTrimmedString(job.wakeMode)) !== "next-heartbeat") patch.wakeMode = "next-heartbeat";
...
function resolveStartupCronSourceFromEvent(event) {
...
function resolveCronServiceFromStartupSource(source) {
	if (!source) return null;
	return resolveCronServiceFromCandidate(source.context.cron) ?? resolveCronServiceFromCandidate(source.deps?.cron);
}
function resolveStartupConfigFromEvent(event, fallback) {
	const startupCfg = asNullableRecord(asNullableRecord(asNullableRecord(event)?.context)?.cfg);
...
function registerShortTermPromotionDreaming(api) {
	...
	const reconcileManagedDreamingCron = async (params) => {
		...
		const config = resolveShortTermPromotionDreamingConfig({
			pluginConfig: resolveMemoryCorePluginConfig(startupCfg) ?? resolveMemoryCorePluginConfig(api.config) ?? api.pluginConfig,
			cfg: startupCfg
		});
