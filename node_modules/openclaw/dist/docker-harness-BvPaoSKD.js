import { c as seedQaAgentWorkspace } from "./gateway-child-DxMrKPS2.js";
...
function renderCompose(params) {
	...
	const qaLabUiMount = toPosixRelative(params.outputDir, path.join(params.repoRoot, "extensions", "qa-lab", "web", "dist"));
	return `services:
  qa-mock-openai:
...
async function writeQaDockerHarnessFiles(params) {
	...
	await Promise.all([
		fs.writeFile(path.join(params.outputDir, "docker-compose.qa.yml"), renderCompose({
		...
		fs.writeFile(path.join(params.outputDir, ".env.example"), renderEnvExample({
			gatewayPort,
			qaLabPort,
			gatewayToken,
			providerBaseUrl,
		...
		fs.writeFile(path.join(params.outputDir, "README.md"), renderReadme({
			gatewayPort,
			qaLabPort,
			usePrebuiltImage,
			bindUiDist,
		...
		fs.writeFile(path.join(params.outputDir, "state", "openclaw.json"), `${JSON.stringify(config, null, 2)}\n`, "utf8")
	]);
	return {
		outputDir: params.outputDir,
		imageName,
...
//#endregion
export { writeQaDockerHarnessFiles as n, buildQaDockerHarnessImage as t };
