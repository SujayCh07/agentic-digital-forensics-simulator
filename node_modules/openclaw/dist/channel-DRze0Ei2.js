...
import { l as normalizeSecretInputString } from "./types.secrets-D-R9X_jm.js";
import { i as createLazyRuntimeNamedExport } from "./lazy-runtime-CnM-CcsI.js";
import { d as createTopLevelChannelConfigAdapter } from "./channel-config-helpers-BcgzHVD7.js";
import "./text-runtime-dYMrRdFH.js";
import { n as describeAccountSnapshot } from "./account-helpers-B1xr09mz.js";
import { c as stripTargetKindPrefix, r as createChatChannelPlugin, s as stripChannelTargetPrefix, t as buildChannelOutboundSessionRoute } from "./core-D3WJPvNW.js";
import "./channel-core-P6YU9jYq.js";
import { t as formatAllowFromLowercase } from "./allow-from-CdVLSPHc.js";
import { E as projectConfigWarningCollector, h as createAllowlistProviderGroupPolicyWarningCollector, n as createDangerousNameMatchingMutableAllowlistWarningCollector } from "./channel-policy-B3TuWXAd.js";
import { t as createChannelDirectoryAdapter } from "./directory-runtime-rgoUBRtX.js";
import { a as listDirectoryEntriesFromSources } from "./directory-config-helpers-pLxRYPjS.js";
import { n as createRuntimeOutboundDelegates, t as createRuntimeDirectoryLiveAdapter } from "./runtime-forwarders-C-4EV5w_.js";
import { Q as splitSetupEntries, f as createStandardChannelSetupStatus, g as createTopLevelChannelGroupPolicySetter, m as createTopLevelChannelDmPolicy, p as createTopLevelChannelAllowFromSetter, v as mergeAllowFromEntries } from "./setup-wizard-helpers-np_EArk_.js";
...
import { d as createDefaultChannelRuntimeState, i as buildProbeChannelStatusSummary, u as createComputedAccountStatusAdapter } from "./status-helpers-wqet6BlE.js";
...
import { n as createMessageToolCardSchema } from "./channel-actions-BcurpI0W.js";
import "./channel-status-BR6xv1a1.js";
import { H as hasConfiguredMSTeamsCredentials, U as resolveMSTeamsCredentials, x as formatUnknownError } from "./graph-users-BaOQ_aB8.js";
import { c as normalizeMSTeamsUserInput, d as parseMSTeamsTeamEntry, f as resolveMSTeamsChannelAllowlist, l as parseMSTeamsConversationId, o as looksLikeMSTeamsTargetId, p as resolveMSTeamsUserAllowlist, r as resolveMSTeamsGroupToolPolicy, s as normalizeMSTeamsMessagingTarget, u as parseMSTeamsTeamChannelInput } from "./policy-C8p8P6Bu.js";
import { t as MSTeamsChannelConfigSchema } from "./config-schema-DZ2RQPfs.js";
import { n as collectRuntimeConfigAssignments, r as secretTargetRegistryEntries } from "./secret-contract-Cezee4wA.js";
import { Type } from "@sinclair/typebox";
//#region extensions/msteams/src/actions.ts
const loadMSTeamsChannelRuntime$2 = createLazyRuntimeNamedExport(() => import("./channel.runtime-Cp6PaINo.js"), "msTeamsChannelRuntime");
	...
	handleAction: async (ctx) => {
		if (ctx.action === "send" && ctx.params.card) {
			...
			return await runWithRequiredActionTarget({
				...
				run: async (to) => {
					const { sendAdaptiveCardMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "upload-file") {
			...
			return await runWithRequiredActionTarget({
				...
				run: async (to) => {
					const { sendMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
					const result = await sendMessageMSTeams({
						cfg: ctx.cfg,
						to,
						text: resolveActionContent(ctx.params),
						mediaUrl,
						filename: normalizeOptionalString(ctx.params.filename) ?? normalizeOptionalString(ctx.params.title),
		...
		if (ctx.action === "edit") {
			...
			return await runWithRequiredActionMessageTarget({
				...
				currentChannelId: ctx.toolContext?.currentChannelId,
				run: async (target) => {
					const { editMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "delete") return await runWithRequiredActionMessageTarget({
			...
			currentChannelId: ctx.toolContext?.currentChannelId,
			run: async (target) => {
				const { deleteMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "read") return await runWithRequiredActionMessageTarget({
			...
			graphOnly: true,
			run: async (target) => {
				const { getMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "pin") return await runWithRequiredActionMessageTarget({
			...
			graphOnly: true,
			run: async (target) => {
				const { pinMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "unpin") return await runWithRequiredActionPinnedMessageTarget({
			...
			graphOnly: true,
			run: async (target) => {
				const { unpinMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "list-pins") return await runWithRequiredActionTarget({
			...
			graphOnly: true,
			run: async (to) => {
				const { listPinsMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "react") return await runWithRequiredActionMessageTarget({
			...
			run: async (target) => {
				...
				if (remove) {
					const { unreactMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
				...
				const { reactMessageMSTeams } = await loadMSTeamsChannelRuntime$2();
				return jsonMSTeamsActionResult("react", {
					reactionType: emoji,
		...
		if (ctx.action === "reactions") return await runWithRequiredActionMessageTarget({
			...
			graphOnly: true,
			run: async (target) => {
				const { listReactionsMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "search") return await runWithRequiredActionTarget({
			...
			run: async (to) => {
				...
				const { searchMessagesMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "member-info") {
			...
			const { getMemberInfoMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "channel-list") {
			...
			const { listChannelsMSTeams } = await loadMSTeamsChannelRuntime$2();
		...
		if (ctx.action === "channel-info") {
			...
			const { getChannelInfoMSTeams } = await loadMSTeamsChannelRuntime$2();
...
const msteamsDirectoryAdapter = createChannelDirectoryAdapter({
	self: async ({ cfg }) => {
	...
	...createRuntimeDirectoryLiveAdapter({
		getRuntime: createLazyRuntimeNamedExport(() => import("./channel.runtime-Cp6PaINo.js"), "msTeamsChannelRuntime"),
...
const loadMSTeamsChannelRuntime = createLazyRuntimeNamedExport(() => import("./channel.runtime-Cp6PaINo.js"), "msTeamsChannelRuntime");
...
const msteamsPlugin = createChatChannelPlugin({
	base: {
		...
		gateway: { startAccount: async (ctx) => {
			const { monitorMSTeamsProvider } = await import("./src-CkyeGQ6z.js");
...
//#endregion
export { msteamsSetupAdapter as i, msteamsSetupWizard as n, createMSTeamsSetupWizardBase as r, msteamsPlugin as t };
