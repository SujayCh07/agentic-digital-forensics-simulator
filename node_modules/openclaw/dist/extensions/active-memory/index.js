...
import { E as parseModelRef } from "../../model-selection-JHakuCn9.js";
import { a as resolveAgentDir, m as resolveAgentWorkspaceDir, o as resolveAgentEffectiveModelPrimary } from "../../agent-scope-BfV3ML7Z.js";
import { r as DEFAULT_PROVIDER } from "../../defaults-CF547YAW.js";
...
function buildPluginStatusLine(params) {
	...
	if (params.result.status === "ok" && params.result.summary.length > 0) parts.push(`${params.result.summary.length} chars`);
	return parts.join(" ");
}
function buildPluginDebugLine(summary) {
...
function stripRecalledContextNoise(text) {
	return text.split("\n").map((line) => line.trim()).filter((line) => {
		...
		if (line.includes(`<${ACTIVE_MEMORY_PLUGIN_TAG}>`) || line.includes(`</${ACTIVE_MEMORY_PLUGIN_TAG}>`)) return false;
...
function getModelRef(api, agentId, config, ctx) {
	...
	const configured = config.model || currentRunModel || agentPrimaryModel || (config.modelFallbackPolicy === "default-remote" ? DEFAULT_MODEL_REF : void 0);
	if (!configured) return;
	const parsed = parseModelRef(configured, DEFAULT_PROVIDER);
	if (parsed) return parsed;
	return (agentPrimaryModel ? parseModelRef(agentPrimaryModel, "openai") : void 0) ?? {
		provider: "openai",
		model: configured
	};
}
async function runRecallSubagent(params) {
	...
	try {
		return {
			rawReply: ((await params.api.runtime.agent.runEmbeddedPiAgent({
...
var active_memory_default = definePluginEntry({
	...
	register(api) {
		...
		const refreshLiveConfigFromRuntime = () => {
			config = normalizePluginConfig(resolveActiveMemoryPluginConfigFromConfig(api.runtime.config.loadConfig()) ?? api.pluginConfig);
		};
		api.registerCommand({
			name: "active-memory",
