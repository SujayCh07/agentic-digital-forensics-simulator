...
import { x as buildMediaPayload } from "./reply-payload-CPJQ0UIg.js";
...
import { n as keepHttpServerTaskAlive } from "./channel-lifecycle.core-7oS5NFHz.js";
import { i as mergeAllowlist, o as summarizeMapping } from "./resolve-utils-o73zmn1N.js";
import { r as resolveDefaultGroupPolicy } from "./runtime-group-policy-Beg3nTTB.js";
import { n as isDangerousNameMatchingEnabled } from "./dangerous-name-matching-DMvuNJ9y.js";
import { a as resolveSenderScopedGroupPolicy, i as evaluateSenderGroupAccessForPolicy } from "./group-access-Dzk6jczG.js";
import { n as readStoreAllowFromForDmPolicy, o as resolveDmGroupAccessWithLists, s as resolveEffectiveAllowFromLists } from "./dm-policy-shared-1zVsdEGf.js";
...
import { t as resolveChannelMediaMaxBytes } from "./media-limits-CbDBZ-Ry.js";
import { t as createChannelReplyPipeline } from "./channel-reply-pipeline-tHqXP07h.js";
import { n as createChannelPairingController } from "./channel-pairing-B4BhGbz7.js";
import { n as DEFAULT_WEBHOOK_MAX_BODY_BYTES } from "./http-body-Dittl_fd.js";
import { n as filterSupplementalContextItems, r as shouldIncludeSupplementalContext } from "./context-visibility-Ccoaoyrv.js";
import { t as resolveChannelContextVisibilityMode } from "./context-visibility-BFfgNy9z.js";
import { r as dispatchReplyFromConfigWithSettledDispatcher } from "./inbound-reply-dispatch-BTtqcSfp.js";
...
import { A as inferPlaceholder, B as safeHostForUrl, C as ATTACHMENT_TAG_RE, D as encodeGraphShareId, E as applyAuthorizationHeaderForUrl, F as readNestedString, I as resolveAttachmentFetchPolicy, L as resolveMediaSsrfPolicy, M as isLikelyImageAttachment, N as isUrlAllowed, O as extractHtmlFromAttachment, P as normalizeContentType, R as resolveRequestUrl, T as IMG_SRC_RE, U as resolveMSTeamsCredentials, V as tryBuildGraphSharesUrlForSharedLink, _ as buildUserAgent, a as fetchGraphJson, b as formatMSTeamsSendErrorHint, g as loadMSTeamsSdkWithAuth, h as createMSTeamsTokenProvider, j as isDownloadableAttachment, k as extractInlineImageCandidates, m as createMSTeamsAdapter, p as createBotFrameworkJwtValidator, v as ensureUserAgentHeader, w as GRAPH_ROOT, x as formatUnknownError, y as classifyMSTeamsSendError, z as safeFetchWithPolicy } from "./graph-users-BaOQ_aB8.js";
import { t as getMSTeamsRuntime } from "./runtime-api-z4nwmllm.js";
import { a as resolveMSTeamsRouteConfig, f as resolveMSTeamsChannelAllowlist, i as resolveMSTeamsReplyPolicy, n as resolveMSTeamsAllowlistMatch, p as resolveMSTeamsUserAllowlist, t as isMSTeamsGroupAllowed } from "./policy-C8p8P6Bu.js";
import { C as writeJsonFile, S as withFileLock, _ as uploadToConsentUrl, b as createMSTeamsConversationStoreFs, c as renderReplyPayloadsToMessages, d as withRevokedProxyFallback, f as resolveGraphChatId, g as parseFileConsentInvoke, h as buildFileInfoCard, l as sendMSTeamsMessages, m as removePendingUpload, p as getPendingUpload, s as buildConversationReference, u as AI_GENERATED_ENTITY, v as createMSTeamsPollStoreFs, w as resolveMSTeamsStorePath, x as readJsonFile, y as extractMSTeamsPollVote } from "./probe-CWA1AdFX.js";
...
async function fetchBotFrameworkAttachmentInfo(params) {
	const { response, release } = await fetchWithSsrFGuard({
		url: `${normalizeServiceUrl(params.serviceUrl)}/v3/attachments/${encodeURIComponent(params.attachmentId)}`,
...
async function fetchBotFrameworkAttachmentView(params) {
	const { response, release } = await fetchWithSsrFGuard({
		url: `${normalizeServiceUrl(params.serviceUrl)}/v3/attachments/${encodeURIComponent(params.attachmentId)}/views/${encodeURIComponent(params.viewId)}`,
		fetchImpl: params.fetchFn ?? fetch,
		init: { headers: ensureUserAgentHeader({ Authorization: `Bearer ${params.accessToken}` }) },
		policy: params.ssrfPolicy,
			...
			if (msgRes.ok) {
				...
				for (const att of spAttachments) {
					...
					try {
						...
						if (!isUrlAllowed(sharesUrl, policy.allowHosts)) continue;
						const media = await downloadAndStoreMSTeamsRemoteMedia({
							url: sharesUrl,
							filePathHint: name,
							maxBytes: params.maxBytes,
...
async function resolveMSTeamsInboundMedia(params) {
	...
	if (mediaList.length === 0) {
		const hasHtmlAttachment = attachments.some((att) => typeof att.contentType === "string" && att.contentType.startsWith("text/html"));
		if (hasHtmlAttachment && isBotFrameworkPersonalChatId(conversationId)) if (!serviceUrl) log.debug?.("bot framework attachment skipped (missing serviceUrl)", {
			conversationType,
			conversationId
		});
		else {
			const attachmentIds = extractMSTeamsHtmlAttachmentIds(attachments);
			if (attachmentIds.length === 0) log.debug?.("bot framework attachment ids unavailable", {
				conversationType,
				conversationId
			});
			else {
