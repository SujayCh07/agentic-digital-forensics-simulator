...
import { o as discoverOpenClawPlugins, s as resolvePluginCacheInputs } from "./ids-BSkPonH4.js";
...
import { g as shouldPreferNativeJiti, n as buildPluginLoaderJitiOptions, t as buildPluginLoaderAliasMap } from "./sdk-alias-Pc_bYsyi.js";
...
function resolvePluginSetupCliBackend(params) {
	...
	const direct = resolvePluginSetupRegistry(params).cliBackends.find((entry) => normalizeProviderId(entry.backend.id) === normalized);
	if (direct) return direct;
	const env = params.env ?? process.env;
	const discovery = discoverOpenClawPlugins({
		workspaceDir: params.workspaceDir,
	...
	const record = loadPluginManifestRegistry({
		...
		candidates: discovery.candidates,
		diagnostics: discovery.diagnostics
	}).plugins.find((entry) => entry.cliBackends.some((backendId) => normalizeProviderId(backendId) === normalized));
	if (!record) return;
	const setupSource = record.setupSource ?? resolveSetupApiPath(record.rootDir);
	...
	const resolved = resolveRegister(mod.default ?? mod);
	if (!resolved.register) return;
	if (resolved.definition?.id && resolved.definition.id !== record.id) return;
	let matchedBackend;
