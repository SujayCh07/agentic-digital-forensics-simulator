...
import { c as optimizeImageToPng, i as convertHeicToJpeg, l as resizeToJpeg, o as hasAlphaChannel } from "./image-ops-Bdlr5m93.js";
import { c as normalizeMimeType, n as extensionForMime, p as maxBytesForKind, s as kindFromMime, t as detectMime } from "./mime-0OurzcCb.js";
import { t as SafeOpenError, u as readLocalFileSafely } from "./fs-safe-DQfR1Er7.js";
...
function isHeicSource(opts) {
	if (HEIC_MIME_RE.test(normalizeOptionalString(opts.contentType) ?? "")) return true;
	if (HEIC_EXT_RE.test(normalizeOptionalString(opts.fileName) ?? "")) return true;
	return false;
}
function assertHostReadMediaAllowed(params) {
	if (params.kind === "image" || params.kind === "audio" || params.kind === "video") return;
	if (params.kind !== "document") throw new LocalMediaAccessError("path-not-allowed", `Host-local media sends only allow images, audio, video, PDF, and Office documents (got ${normalizeMimeType(params.contentType) ?? "unknown"}).`);
...
async function optimizeImageWithFallback(params) {
	...
	if ((meta?.contentType === "image/png" || normalizeLowercaseStringOrEmpty(meta?.fileName).endsWith(".png")) && await hasAlphaChannel(buffer)) {
