...
import { o as resolveProviderHttpRequestConfig, r as postJsonRequest, t as assertOkOrThrowHttpError } from "./shared-D_aQ0EvE.js";
...
import { a as resolveConfiguredOpenAIBaseUrl, o as toOpenAIDataUrl } from "./shared-4OVUeWWx.js";
...
async function pollOpenAIVideo(params) {
	for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt += 1) {
		...
		if (payload.status === "failed") throw new Error(normalizeOptionalString(payload.error?.message) || "OpenAI video generation failed");
		...
		async generateVideo(req) {
			...
			const { response, release } = referenceAsset ? inputImage?.buffer ? await (() => {
				...
				return fetchWithTimeout(requestUrl, {
					...
					body: form
				}, req.timeoutMs ?? DEFAULT_TIMEOUT_MS, fetchFn).then((response) => ({
					response,
					release: async () => {}
				}));
			})() : await (() => {
